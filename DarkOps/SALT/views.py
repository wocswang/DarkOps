            ...
            if env:
                if file:
                    if content:#写入文件内容，文件会产生[noeol]问题，因此需要在内容最后加个结束符\n
                        ...
                        if success:
                            result="文件%s保存成功！"%file
                        else:
                            result="文件%s保存失败！"%file
                    else:#读取环境下文件内容
...
def minions(request,server_id):
    ...
    try:
        ...
        for m in minion_list:
            ...
            if not m.grains:
                obj={'id':m.id,'minion':m.minion,'ip':'','os':'','status':m.status}
            ms.append(obj)
        ...
        try:
            if status=='Accepted':
                ...
                if grains:
                    ...
                    #print('Minion.grains', Minion.grains)
                if pillar:
                    Minion.pillar=json.dumps(pillar)
                    #print('Minion.pillar', Minion.pillar)
        except Exception as e:
            pass
